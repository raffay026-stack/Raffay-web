    ...
    return (
      <div className="min-h-screen bg-[#0A0A0A] text-neutral-100 font-serif">
        ...
        <div className="max-w-4xl mx-auto px-4 py-32 text-center space-y-6">
          <div className="w-20 h-20 mx-auto rounded-full bg-[#12100C] border border-[#D4AF37]/30 flex items-center justify-center text-[#D4AF37]">
            <ShoppingBag className="w-10 h-10 opacity-60" />
          </div>
          <h2 className="text-2xl font-bold text-[#F3E5AB]">Your Cart is Currently Empty</h2>
          <p className="text-xs text-neutral-400">Add luxury perfumes to proceed to secure checkout.</p>
          <button
            onClick={() => navigate("/catalog")}
            className="px-8 py-3 bg-gradient-to-r from-[#D4AF37] to-[#AA7C11] text-[#0A0A0A] font-serif text-xs font-bold uppercase tracking-widest rounded"
          >
            Browse Collection
          </button>
        </div>
    ...
    <div className="min-h-screen bg-[#0A0A0A] text-neutral-100 font-serif selection:bg-[#D4AF37] selection:text-[#0A0A0A]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="text-center max-w-2xl mx-auto mb-12 space-y-2">
          <div className="inline-flex items-center gap-1.5 text-xs text-[#D4AF37] uppercase tracking-widest">
            <Lock className="w-3.5 h-3.5" />
            <span>256-Bit Encrypted Secure Checkout</span>
          </div>
          <h1 className="text-3xl sm:text-4xl font-serif font-bold text-[#F3E5AB]">
            Haute Parfumerie Checkout
          </h1>
        </div>
        ...
        <form onSubmit={handleSubmitOrder} className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-7 space-y-8">
            ...
            <div className="bg-[#12100C] border border-[#D4AF37]/30 p-6 sm:p-8 rounded-lg space-y-6 shadow-xl">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-serif">
                <div className="space-y-1.5 sm:col-span-2">
                  ...
                  <input 
                    ...
                    className="w-full bg-[#0A0A0A] border border-[#D4AF37]/30 px-3 py-2.5 text-neutral-100 focus:outline-none focus:border-[#D4AF37] rounded"
            ...
            <div className="bg-[#12100C] border border-[#D4AF37]/30 p-6 sm:p-8 rounded-lg space-y-6 shadow-xl">
              ...
              <div className="space-y-3 font-serif text-xs">
                <label className={`flex items-center gap-3 p-4 rounded border cursor-pointer transition-all ${formData.paymentMethod === 'credit-card' ? 'bg-[#1C1610] border-[#D4AF37]' : 'bg-[#0A0A0A] border-neutral-800'}`}>
                  <input 
                    type="radio" 
                    name="paymentMethod" 
                    value="credit-card"
                ...
                <label className={`flex items-center gap-3 p-4 rounded border cursor-pointer transition-all ${formData.paymentMethod === 'online' ? 'bg-[#1C1610] border-[#D4AF37]' : 'bg-[#0A0A0A] border-neutral-800'}`}>
                  <input 
                    type="radio" 
                    name="paymentMethod" 
                    value="online"
                  ...
                  <div className="flex items-center justify-between flex-1">
                    ...
                    <span className="text-[#D4AF37]">Instant</span>
                  </div>
                </label>

                <label className={`flex items-center gap-3 p-4 rounded border cursor-pointer transition-all ${formData.paymentMethod === 'cod' ? 'bg-[#1C1610] border-[#D4AF37]' : 'bg-[#0A0A0A] border-neutral-800'}`}>
                  <input 
                    type="radio" 
                    name="paymentMethod" 
                    value="cod"
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-[#12100C] border border-[#D4AF37]/40 p-6 sm:p-8 rounded-lg shadow-2xl space-y-6 sticky top-28">
              
              <h3 className="text-lg font-serif font-bold text-[#F3E5AB] border-b border-neutral-800 pb-4">
              ...
              <div className="max-h-72 overflow-y-auto space-y-4 pr-1">
                {cart.map((item) => (
                  <div key={`${item.id}-${item.selectedSize}`} className="flex items-center gap-3 pb-3 border-b border-neutral-800/60">
                    <img src={item.image} alt={item.name} className="w-14 h-14 object-cover rounded border border-[#D4AF37]/30" />
                    <div className="flex-1">
                      <div className="text-[10px] text-[#D4AF37] uppercase tracking-wider font-serif">{item.brand}</div>
                      <div className="font-serif text-xs text-neutral-200 line-clamp-1">{item.name}</div>
                      <div className="text-[11px] text-neutral-400 font-serif">Size: {item.selectedSize} × {item.quantity}</div>
                    </div>
              ...
              <button
                ...
                className="w-full py-4 bg-gradient-to-r from-[#F3E5AB] via-[#D4AF37] to-[#AA7C11] text-[#0A0A0A] font-serif text-xs font-bold uppercase tracking-widest rounded shadow-2xl hover:opacity-95 transition-all flex items-center justify-center gap-2 group"
