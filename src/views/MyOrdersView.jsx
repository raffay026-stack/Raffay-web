    ...
    <div className="min-h-screen bg-[#0A0A0A] text-neutral-100 font-serif selection:bg-[#080809] selection:text-[#0A0A0A]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 pb-6 border-b border-neutral-800 gap-4">
          <div>
            <div className="inline-flex items-center gap-1.5 text-xs text-[#080809] uppercase tracking-widest mb-1">
          ...
          <div className="flex gap-2">
            {["all", "processing", "shipped", "delivered"].map((st) => (
              <button
                ...
                className={`px-4 py-2 rounded text-xs uppercase tracking-widest font-serif transition-all ${selectedStatus === st ? 'bg-[#080809] text-[#0A0A0A] font-bold shadow-lg' : 'bg-[#12100C] text-neutral-400 border border-[#080809]/30 hover:border-[#080809]'}`}
              >
                {st}
              </button>
            ))}
        ...
        {filteredOrders.length === 0 ? (
          <div className="text-center py-24 bg-[#12100C] border border-[#080809]/20 rounded-lg space-y-4">
            ...
            <button
              ...
              className="px-6 py-2.5 bg-gradient-to-r from-[#080809] to-[#46CDB3] text-[#0A0A0A] font-serif text-xs font-bold uppercase tracking-widest rounded shadow"
            >
              Explore Collection
            </button>
          </div>
          ...
          <div className="space-y-6">
            {filteredOrders.map((order) => (
              <div 
                key={order.id}
                className="bg-gradient-to-b from-[#12100C] to-[#0A0A0A] border border-[#080809]/30 rounded-lg p-6 sm:p-8 shadow-xl space-y-6"
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-4 border-b border-neutral-800 gap-4">
                  <div className="space-y-1">
                    ...
                    <h3 className="text-lg font-bold text-[#080809]">#{order.id}</h3>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      ...
                      <span className="text-xs px-3 py-1 rounded bg-[#080809]/20 text-[#080809] border border-[#080809]/40 font-bold inline-flex items-center gap-1 mt-0.5">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-[#1C1610] border border-[#080809]/40 text-[#080809] font-serif text-xs uppercase tracking-wider rounded hover:bg-[#282117] transition-all flex items-center gap-1.5"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {order.items?.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-3 p-3 rounded bg-[#0A0A0A] border border-[#080809]/20">
                      <img src={item.image} alt={item.name} className="w-12 h-12 object-cover rounded border border-[#080809]/30" />
                      <div className="flex-1">
                        <div className="text-[9px] text-[#080809] uppercase tracking-wider">{item.brand}</div>
                        <div className="font-serif text-xs font-medium text-neutral-200 line-clamp-1">{item.name}</div>
                        <div className="text-[10px] text-neutral-400">Size: {item.selectedSize} Ã— {item.quantity}</div>
                      </div>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center pt-4 border-t border-neutral-800 text-xs font-serif gap-2">
                  ...
                  <div className="flex items-center gap-4">
                    <span className="text-neutral-400">Grand Total: <strong className="text-[#080809] text-sm">${order.grandTotal?.toFixed(2)}</strong></span>
                  </div>
                </div>
