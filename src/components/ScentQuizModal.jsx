  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md" data-testid="style-quiz-modal">
      <div className="relative w-full max-w-xl bg-gradient-to-b from-[#16130D] to-[#0A0A0A] border border-[#080809]/50 rounded-lg p-6 sm:p-8 text-neutral-100 shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <button 
          onClick={onClose}
          className="absolute top-4 right-4 text-neutral-400 hover:text-[#080809] transition-colors"
            ...
            <div className="flex justify-between items-center mt-8 text-xs text-neutral-500 font-serif">
              ...
              <div className="flex gap-1">
                {SCENT_QUIZ_QUESTIONS.map((_, i) => (
                  <div 
                    key={i} 
                    className={`w-6 h-1 rounded-full ${i <= currentStep ? 'bg-[#080809]' : 'bg-neutral-800'}`} 
                  />
                ))}
              </div>
            </div>
          ...
          <div className="space-y-6 text-center animate-in fade-in duration-300">
            ...
            <div className="flex gap-3 pt-4">
              <button
                ...
                className="flex-1 py-2.5 bg-[#1C1710] border border-[#2FB59A]/40 text-[#9AE9D8] font-serif text-xs uppercase tracking-widest rounded hover:bg-[#282117] transition-all flex items-center justify-center gap-2"
              ...
              <button
                ...
                className="flex-1 py-2.5 bg-gradient-to-r from-[#2FB59A] to-[#2FB59A] text-[#9AE9D8] font-serif text-xs font-bold uppercase tracking-widest rounded hover:opacity-95 transition-all flex items-center justify-center gap-2"
              >
