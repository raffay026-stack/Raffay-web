  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden" data-testid="cart-drawer-modal">
      ...
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-md bg-[#0D0B08] border-l border-[#43111F]/30 text-[#2D2326] flex flex-col shadow-2xl animate-in slide-in-from-right duration-300">
          
          {/* Header */}
          <div className="p-6 border-b border-[#43111F]/20 flex items-center justify-between bg-[#F7F1EC]">
            <div className="flex items-center gap-3">
              <span className="font-serif text-lg font-bold tracking-widest text-[#43111F]">
                CRYSTAL CART
              </span>
              <span className="text-xs px-2 py-0.5 rounded-full bg-[#43111F]/20 text-[#43111F] border border-[#43111F]/40">
                {cart.reduce((s, i) => s + i.quantity, 0)} Items
              </span>
            </div>
            <button 
              onClick={onClose}
              className="p-1 rounded-full text-[#7C6E72] hover:text-[#43111F] transition-colors"
            ...
            {cart.length === 0 ? (
              <div className="text-center py-20 space-y-4">
                ...
                <button
                  ...
                  className="mt-4 px-6 py-2.5 bg-gradient-to-r from-[#6E1F35] to-[#6E1F35] text-[#6E1F35] font-serif text-xs font-bold uppercase tracking-widest rounded-sm hover:opacity-95 transition-all"
              ...
              cart.map((item) => (
                <div 
                  ...
                  className="flex gap-4 p-4 rounded-lg bg-[#F7F1EC]/80 border border-[#43111F]/20 relative group"
                >
                  <img 
                    src={item.image} 
                    alt={item.name} 
                    className="w-20 h-20 object-cover rounded-md border border-[#43111F]/30 bg-neutral-900"
                  />
                  <div className="flex-1 flex flex-col justify-between">
                    ...
                    <div className="flex items-center justify-between mt-3">
                      <div className="flex items-center border border-[#43111F]/40 rounded bg-[#FFFDF8]">
                        <button 
                          onClick={() => updateCartQuantity(item.id, item.selectedSize, item.quantity - 1)}
                          className="p-1 text-[#7C6E72] hover:text-[#43111F]"
                          data-testid={CART.itemQtyMinus}
                        >
                          <Minus className="w-3.5 h-3.5" />
                        </button>
                        <span className="px-3 text-xs font-serif text-[#43111F]">{item.quantity}</span>
                        <button 
                          onClick={() => updateCartQuantity(item.id, item.selectedSize, item.quantity + 1)}
                          className="p-1 text-[#7C6E72] hover:text-[#43111F]"
                  ...
                  <button 
                    ...
                    className="absolute top-2 right-2 text-[#8A7A80] hover:text-red-400 p-1 transition-colors"
                    title="Remove item"
            ...
            <div className="p-6 border-t border-[#43111F]/20 bg-[#120F0A] space-y-4">
              ...
              <div className="flex gap-2">
                <input 
                  ...
                  className="flex-1 bg-[#FFFDF8] border border-[#43111F]/30 px-3 py-2 text-xs font-serif text-[#2D2326] focus:outline-none focus:border-[#43111F]"
